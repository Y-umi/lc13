...
/obj/machinery/chem_master/proc/get_condi_styles()
	...
	if (!styles.len)
		...
		styles += list(
			...
			"milk" = list("icon_state" = "milk", "icon_empty" = "", "name" = "milk", "desc" = "It's milk. White and nutritious goodness!"),
...
/datum/player_reagent_list/proc/generate_chem_guidebook_data()
	...
	for(var/chem_path in generic_chem_list)
		...
		.["chems"] += list(list(
			...
			// Armor
			"red" = abno_chem.armor_mods[1] != 0 ? armor_to_protection_class(abno_chem.armor_mods[1]) : abno_chem.armor_mods[1],
			"whi" = abno_chem.armor_mods[2] != 0 ? armor_to_protection_class(abno_chem.armor_mods[2]) : abno_chem.armor_mods[2],
			"bla" = abno_chem.armor_mods[3] != 0 ? armor_to_protection_class(abno_chem.armor_mods[3]) : abno_chem.armor_mods[3],
			"pal" = abno_chem.armor_mods[4] != 0 ? armor_to_protection_class(abno_chem.armor_mods[4]) : abno_chem.armor_mods[4],
			// Damage Taken Mods
