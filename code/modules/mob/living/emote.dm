...
/datum/emote/living/kiss/run_emote(mob/living/user, params, type_override, intentional)
	...
	if(user.put_in_hands(kiss_blower))
		...
		qdel(kiss_blower)
		to_chat(user, span_notice("You're incapable of blowing a kiss in your current state."))
...
/datum/emote/living/laugh/get_sound(mob/living/user)
	if(ishuman(user))
		var/mob/living/carbon/human/H = user
		//if(H.dna.species.id == SPECIES_HUMAN && (!H.mind || !H.mind.miming)) REQUIRES PR #59797
		...
		if(custom_emote && !check_invalid(user, custom_emote))
			...
			switch(type)
				...
				if("Hearable")
					...
					tgui_alert(user, "Unable to use this emote, must be either hearable or visible.")
					return
	else
		custom_emote = params
		if(type_override)
